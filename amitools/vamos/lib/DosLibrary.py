...
class DosLibrary(LibImpl):
    ...
    def setup_lib(self, ctx, base_addr):
        ...
        self.timeRequest = ctx.alloc.alloc_astruct(
            TimeRequestStruct, label="TimeRequest"
        )
        self.timeRequest.tr_node.io_Device.aptr = self.timerDevice
        self.lib_struct.dl_TimeReq.aptr = self.timeRequest.addr
    ...
    def GetVar(self, ctx):
        ...
        if not flags & self.GVF_GLOBAL_ONLY:
            node = ctx.process.find_var(name, flags & 0xFF)
            if node != None:
    ...
    def FindVar(self, ctx):
        ...
        name = ctx.mem.r_cstr(name_ptr)
        node = ctx.process.find_var(name, vtype & 0xFF)
        if node == None:
            self.setioerr(ctx, ERROR_OBJECT_NOT_FOUND)
    ...
    def SetVar(self, ctx):
        ...
        if buff_ptr == 0:
            if not flags & self.GVF_GLOBAL_ONLY:
                node = ctx.process.find_var(name, vtype)
                if node != None:
                    ctx.process.delete_var(node)
                return DOSTRUE
        else:
            if flags & self.GVF_BINARY_VAR:
                value = None
            ...
            if not flags & self.GVF_GLOBAL_ONLY:
                proc = ctx.process
                node = proc.find_var(name, vtype)
                if node == None:
                    node = proc.create_var(name, vtype)
                if flags & self.GVF_BINARY_VAR:
                    proc.set_var_value(node, size, src_addr=buff_ptr)
                else:
                    proc.set_var_value(node, size, value=value)
                return DOSTRUE
        return 0
    ...
    def SystemTagList(self, ctx, cmd: CSTR, tag_list: TagList):
        ...
        if ctx.process.is_native_shell():
            ...
            if not proc.ok:
                ...
                return DOSTRUE
            # a child inherits the parent's local variables (NP_CopyVars)
            cur_proc.copy_local_vars(proc.get_local_vars())
    ...
    def CreateNewProc(self, ctx, tags: TagList):
        ...
        varlist = MinListStruct(ctx.mem, proc.local_vars.addr)
        varlist.mlh_Head.aptr = varlist.addr + 4
        varlist.mlh_Tail.aptr = 0
        varlist.mlh_TailPred.aptr = varlist.addr
        if tags.get_tag_data(DosTag.NP_CopyVars, 1) and ctx.process:
            ctx.process.copy_local_vars(varlist)
